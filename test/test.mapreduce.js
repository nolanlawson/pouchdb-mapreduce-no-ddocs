...
var stringMd5 = require('pouchdb-md5').stringMd5;
...
adapters.forEach(function (adapter) {
  ...
  testConfigs.forEach(function (config) {
    var suiteName = 'test.mapreduce.js-' + adapter + '-' + config.viewType +
      '-auto=' + config.auto;
    var dbName = testUtils.adapterUrl(adapter, 'testdb');

    tests(suiteName, dbName, adapter, config.viewType, config.auto);
  });
});

function tests(suiteName, dbName, dbType, viewType, auto) {
  ...
  describe(suiteName, function () {
    ...
    if (viewType === 'persisted' && !auto) {
      ...
      createView = function (db, viewObj) {
        if (auto) {
          viewObj.saveAs = 'custom-' + stringMd5(viewObj.toString());
        }
      ...
      return createView(db, {
        ...
        reduce: function (keys, values, rereduce) {
          ...
          if (!rereduce) {
            ...
            var thisSum = sum(values.map(function (value) {
              return value.sum;
            }));
            var thisCount = sum(values.map(function (value) {
              return value.count;
            }));
            return {
              sum: thisSum,
    ...
    it('should update the emitted value', function () {
      ...
      return createView(db, {
        ...
        return db.bulkDocs({docs: docs}).then(function (res) {
          ...
          var values = res.rows.map(function (x) {
            return x.value;
          });
